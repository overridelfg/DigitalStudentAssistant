...
class CVRepositoryImpl(
    ...
    override suspend fun addCV(cv: CV): OperationResult<CV, String?> {
        return withContext(Dispatchers.IO) {
            try {
                val result = apiService.addCV(userPrefsStorage.loadUserFromPrefs()!!.token, CVRequest(cv.nameCV)).toCV()
                return@withContext OperationResult.Success(result)
            } catch (e: Throwable) {
                return@withContext OperationResult.Error(e.message)
            }
